...
# # ------------------ Main App Logic ------------------
# uploaded_file = st.file_uploader("Upload your research paper (PDF)", type=["pdf"], key= 'file_uploader', help="Maximum file size: 10 MB")
...
if uploaded_file:
    # Check file size first - STOP processing if too large
    if uploaded_file.size > MAX_FILE_SIZE:
        st.error("File too large! Please upload a PDF under 10 MB.")
        st.info("👆 Please upload a smaller PDF file to continue.")
        # Clear any existing file data from session state
        keys_to_clear = [
            "current_file_hash", "current_filename", "file_processed", 
            "current_retriever", "current_llm", "summary_generated", "split_docs"
        ]
        for key in keys_to_clear:
            if key in st.session_state:
                del st.session_state[key]
        st.stop()  # This prevents any further execution
    
    # File size is acceptable - continue processing
    new_file_hash = file_hash(uploaded_file)
    ...
    # Clear any existing file data when no file is present
    keys_to_clear = ["current_file_hash", "current_filename", "file_processed"]
    for key in keys_to_clear:
        if key in st.session_state:
            del st.session_state[key]
